...
public class ResourceController {
	...
	@RequestMapping("findAllResourceStatus")
	public List<Resource> findALlStatus(){
		ResourceExample example = new ResourceExample();
		example.createCriteria().andBak2EqualTo("1");
		return ResourceService.findALlStatus(example);
	}
	...
	public Result add(Resource Resource){
		try {
			if(Resource.getId()==null){
				...
				Resource.setCreat_date(DateUtils.getNowDate(new Date()));
